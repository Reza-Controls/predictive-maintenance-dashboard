...
**While `modbus_client.py` runs, it automatically regenerates two files every 10
readings** (and once more on a clean Ctrl+C exit):
- `screenshots/dashboard_output.gif` — an animated chart of the run so far, with
  anomalies marked, ready to embed in this README or a portfolio page.
- `results_table.md` — a live markdown table of readings/anomaly counts, ready
  to paste straight into a README's Results section (see below).

No manual screen recording or copy-typing numbers required — just let it run
for a few minutes and both files are current.
...
Both sections below are auto-generated (not hand-typed) — `modbus_client.py`
writes the first as it runs, and `ml_anomaly_detector.py` writes the second
when you run it afterward. Copy the current content of `results_table.md`
here once you've done a real run:

<!-- LIVE_METRICS_START -->
## Results (live, z-score detector)

*Auto-generated by modbus_client.py*

| Metric | Value |
|---|---|
| Total readings analyzed | 260 |
| Anomalies flagged (any sensor) | 32 (12.3%) |
| Vibration anomalies | 7 |
| Temperature anomalies | 13 |
| Current anomalies | 18 |
<!-- LIVE_METRICS_END -->

<!-- ML_METRICS_START -->
## Results (Isolation Forest vs. z-score comparison)

*Auto-generated by ml_anomaly_detector.py — last updated 2026-07-21 00:45:22 UTC*

| Metric | Value |
|---|---|
| Total readings analyzed | 263 |
| Flagged by both methods | 17 |
| Flagged by z-score only | 16 |
| Flagged by Isolation Forest only | 10 |
| Flagged by neither (normal) | 220 |
<!-- ML_METRICS_END -->
